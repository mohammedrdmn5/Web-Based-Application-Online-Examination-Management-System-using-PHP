...
if (isset($_GET['edit'])) {
    $exam_id = $_GET['edit'];
    $update = true;
    $num = mysqli_field_count($conn, "SELECT * FROM exam INNER JOIN faculty ON exam.faculty_id = faculty.faculty_id INNER JOIN teacher ON exam.teacher_id = teacher.teacher_id INNER JOIN user ON teacher.user_id = user.user_id INNER JOIN course ON exam.course_id = course.course_id WHERE exam_id=$exam_id") or die($conn->error);
    $result = mysqli_query($conn, "SELECT * FROM exam INNER JOIN faculty ON exam.faculty_id = faculty.faculty_id INNER JOIN teacher ON exam.teacher_id = teacher.teacher_id INNER JOIN user ON teacher.user_id = user.user_id INNER JOIN course ON exam.course_id = course.course_id WHERE exam_id=$exam_id") or die($conn->error);
    if ($num == 1) {
