    ...
    <div id="wrapper">
        ...
        <div class="d-flex flex-column" id="content-wrapper">
                                ...
                                <table class="table my-0" id="dataTable">
                                    ...
                                    <tbody>
                                        ...
                                        if (isset($_POST['txtSearch'])) {
                                            ...
                                            $courseData = mysqli_query($conn, "SELECT course.course_id, course.course_title, faculty.faculty_title, user.fname, user.lname FROM course INNER JOIN faculty ON course.faculty_id = faculty.faculty_id INNER JOIN teacher ON course.teacher_id = teacher.teacher_id AND teacher.faculty_id = faculty.faculty_id INNER JOIN user ON teacher.user_id = user.user_id where course.course_title LIKE  '%$searchTarget%'") or die(mysqli_error($conn));
            ...
            <td>' . $row['fname'] . ' ' . $row['lname'] . '</td>
                                            ...
                                            $courseData = mysqli_query($conn, "SELECT course.course_id, course.course_title, faculty.faculty_title, user.fname, user.lname FROM course INNER JOIN faculty ON course.faculty_id = faculty.faculty_id INNER JOIN teacher ON course.teacher_id = teacher.teacher_id AND teacher.faculty_id = faculty.faculty_id INNER JOIN user ON teacher.user_id = user.user_id") or die(mysqli_error($conn));
                                            // while (mysqli_num_rows($courseData)) {
                                            // }
                                            while ($row = mysqli_fetch_array($courseData)) {
                                                echo '<tr>
