                            ...
                            <div class="table-responsive table mt-2" id="dataTable" role="grid" aria-describedby="dataTable_info">
                                <table class="table my-0" id="dataTable">
                                    ...
                                    if (isset($_GET['search'])) {
                                        ...
                                        $category = $_GET['cat'];
                                        $searchResult = $conn->query(" SELECT CONCAT(user.fname, ' ',user.lname) AS fullname,exam.*,teacher.*,user.*,course.*,faculty.* FROM exam INNER JOIN teacherON exam.teacher_id = teacher.teacher_idINNER JOIN facultyON exam.faculty_id = faculty.faculty_idAND teacher.faculty_id = faculty.faculty_idINNER JOIN courseON exam.course_id = course.course_idAND course.teacher_id = teacher.teacher_idAND course.faculty_id = faculty.faculty_idINNER JOIN userON teacher.user_id = user.user_id WHERE $category LIKE '%$searchValue%'") or die($conn->error);
