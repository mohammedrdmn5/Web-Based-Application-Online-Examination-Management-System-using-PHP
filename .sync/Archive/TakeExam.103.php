...
if (isset($_POST['Submitdata'])) {
    ...
    for ($x = 0; $x < $questions; $x++) {
        ...
        $queryToStudentRec = mysqli_query($conn, "INSERT INTO student_record (user_id, exam_id, question_id, answer, mark)  VALUES ('$user_id','$exam_id1','$question_id','$question_id','$answer_option','$mark')") or die(mysqli_error($conn));
                                ...
                                <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                                    ...
                                    <table class="table my-0" id="dataTable">
                                        ...
                                        if (isset($_GET['search'])) {
                                            ...
                                            $searchValue = $_GET['search'];
                                            //echo "<script> alert('GET >>>>> $searchValue');</script>";
                                            ...
                                            while ($row = $searchResult->fetch_assoc()) {
                                              ...
                                              <input type="hidden" name="exam_id" value="' . $row['exam_id'] . '">
                                                ...
                                                for ($i = 0; $i < $numberOfQs; $i++) {
                                                    ...
                                                    console_log($table_Question_arr);
                                                        //console_log( $row);
                                                        //INSERT INTO `question`(`question_id`, `question_title`, `answer_option`, `mark`, `exam_id`) VALUES ([value-1],[value-2],[value-3],[value-4],[value-5])
                                                        //INSERT INTO `question_opetions`(`opetion_id`, `question_id`, `opetion_title`, `opetion_number`) VALUES ([value-1],[value-2],[value-3],[value-4])
                                                    echo '
                                                    <input type="hidden" name="question_id'.$i.'" value="' . $table_Question_arr['question_id'] . '">
                                                    ';
                                                    echo
                                                    '<tr>
                                                    <td> 
                                                    <label style="color:black;" name="question_title' . $i . '"><h4> Question No. ' . $i +1 . '</h4> <br/>  <h5> '. $table_Question_arr['question_title']. '<h6></label>                                                   
                                                    </td>';
                                                    echo '<td>';
                                                    ...
                                                    while ($table_question_opetions_arr = mysqli_fetch_array($question_opetions_table)) {
                                                        $count += 1;
                                                        //console_log($table_question_opetions_arr);
                                                    ...
                                                    echo '</td>';
                                                    echo '<td>
                                                <label style="color:black;"><h5> Select your answer NO. </h5> &nbsp; <select class="form-control form-control-sm custom-select custom-select-sm" name="answer_option' . $i . '" style="width:fit-content;">
                                                ...
                                                ';
                                                    echo '<td>
                                                <label style="color:black;" name="question_title' . $i . '"><br/> <h5>  ' . $table_Question_arr['mark'] . ' marks </h5> </label> 
                                                </td>
                                                ';
                                                    echo '</tr>';
