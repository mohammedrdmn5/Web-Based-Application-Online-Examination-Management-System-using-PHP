    ...
    for ($x = 0; $x < $questions; $x++) {
        ...
        if ($success) {
            ...
            for ($option = $x; $option < $x + 4; $option++) {
                $opetion_title = $_POST['opetion_title' . $x . ''];
                $count += 1;
                $ques_id = $count -1;
            $success2 = mysqli_query($conn, "INSERT INTO question_opetions (question_id, opetion_title, opetion_number) VALUES ('$question_id[$ques_id]','$opetion_title.$x','$mark.$x','$x')") or die(mysqli_error($conn));
            }
            $count = 0;
