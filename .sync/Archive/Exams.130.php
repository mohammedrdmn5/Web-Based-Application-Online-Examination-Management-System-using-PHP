...
{
    if (isset($exam_id) && $exam_id == $data_id) {
                                        ...
                                        <<input type="hidden" name="id" value="<?php echo $exam_id; ?>">
                                            <div class="input-group" style="padding-top: 20px;">
                                                <input type="text" name="exam_title" class="form-control" required placeholder="Enter exam title" value="<?php echo $exam_title;
                                                                                                                                                            ?>">
                                                <input type="date" data-type="datetime" min="<?php echo date('Y-m-d'); ?>" required name="exam_datetime" class="form-control" placeholder="Enter exam datetime" value="<?php echo $exam_datetime;
                                                                                                                                                                                                                        ?>">
                                                <input type="number" name="exam_duration" class="form-control" required placeholder="Enter exam duration" value="<?php echo $exam_duration;
                                                                                                                                                                    ?>">
                                                <input type="number" name="total_question" class="form-control" required placeholder="Enter total question" value="<?php echo $total_question;
                                                                                                                                                                    ?>">
                                                ...
                                                <select class="form-control" id="sel1" required name="course_title">
                                                ...
                                                <select class="form-control" id="sel1" required name="faculty_title">
                                                    <option value="">SELECT course title</option>
                                                    <?php
                                                    $cfaculty_data = GetDataFromDataBase("SELECT DISTINCT * FROM faculty");
                                                    while ($data = mysqli_fetch_array($cfaculty_data))
                                                        ShowDataToDorpDownlist($data['faculty_title']);
                                                    ?>
                                                </select>
                                                <select class="form-control" id="sel1" required name="fullname">
                                                    <option value="">SELECT course title</option>
                                                    <?php
                                                    $user_data = GetDataFromDataBase("SELECT DISTINCT  *,  CONCAT(fname , ' ', lname) AS fullname FROM user");
                                                    while ($data = mysqli_fetch_array($user_data))
                                                        ShowDataToDorpDownlist($data['fullname']);
                                                    ?>
                                                </select>
                                        ...
                                        while ($row = $searchResult->fetch_assoc()) {
                                            ...
                                            echo '
                                                <tr>
                                                    ...
                                                    <td>' . date_format($row['exam_datetime'], "Y/m/d H:i:s") . '</td>
                                                    <td>' . $row['exam_duration'] . ' Minutes</td>
                                                    <td>' . $row['total_question'] . '</td>
                                                    <td>' .  date_format($row['created_on'], "Y/m/d H:i:s")  . '</td>
                                                    <td>' . $status . '</td>
