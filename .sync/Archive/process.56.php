...
if (isset($_GET['edit'])) {
    $exam_id = $_GET['edit'];
    $update = true;
    $num = mysqli_num_rows($conn, "SELECT * FROM Exam WHERE exam_id=$exam_id") or die($conn->error);

    $result = mysqli_query($conn, "SELECT * FROM Exam WHERE exam_id=$exam_id") or die($conn->error);
    if ($num == 1) {
