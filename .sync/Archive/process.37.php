...
if (isset($_POST['save'])) {
    ...
    if (empty($_POST['exam_title'])) {
        ...
        echo "<script>
           alert('INSERTION ".$teacher_id[0] . " " . $faculty_id[0] . " " . $course_id[0] . "');
        </script>";
        $success = mysqli_query($conn, "INSERT INTO exam (exam_title, exam_datetime, exam_duration, total_question, status, course_id, teacher_id, faculty_id, exam_code) VALUES ('$exam_title','$exam_datetime','$exam_duration','$total_question','$status','$course_id[0]','$teacher_id[0]','$faculty_id[0]','$exam_code')") or die(mysqli_error($conn));
        if ($success) {
